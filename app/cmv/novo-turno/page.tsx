...
export default function NovoTurnoPage() {
  ...
  const updateProduto = useCallback((productId: string, field: keyof TurnoProdutoForm, value: number) => {
    setProdutos(prev => {
      ...
      novo[globalIndex] = { ...novo[globalIndex], [field]: newValue };
      return novo;
    });
  }, []);
  ...
  const totais = useMemo(() => {
    ...
    const produtosCalc = produtos.map(p => {
      const consumo = Math.max(0, p.estoqueInicial + p.entradas - p.estoqueFinal);
  ...
  const handleSalvar = useCallback(async () => {
    ...
    const turnoPayload = {
      ...
      produtos: totais.produtosCalc.filter(p => p.consumo > 0 || p.entradas > 0).map(p => ({
        produtoNome: p.nome,
      ...
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
           <div>
             ...
             <p style={{ color: '#64748b', marginTop: '4px' }}>
               Olá, <span style={{ color: '#4f46e5', fontWeight: 700 }}>{responsavel}</span>! Registre as entradas e o estoque final.
             </p>
           </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1fr_350px] gap-8 items-start">
          ...
          <div className="space-y-6">
            ...
            <div style={{ background: '#fff', borderRadius: '24px', padding: '24px', boxShadow: '0 4px 20px rgba(0,0,0,0.03)', border: '1px solid #e2e8f0' }}>
              <div className="grid grid-cols-1 md:grid-cols-[2fr_1fr_1fr] gap-5">
                ...
                <div>
                  ...
                  <div style={{ width: '100%', padding: '12px', borderRadius: '12px', border: '1px solid #e2e8f0', background: '#f8fafc', color: '#64748b', fontWeight: 600 }}>
                    <i className="fas fa-calendar" style={{ marginRight: '8px' }}></i> {new Date(data).toLocaleDateString('pt-BR')}
                  </div>
                </div>
                <div>
            ...
            {/* Tabela de Insumos */}
            <div style={{ background: '#fff', borderRadius: '24px', overflow: 'hidden', boxShadow: '0 4px 20px rgba(0,0,0,0.03)', border: '1px solid #e2e8f0' }}>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead>
                    <tr style={{ background: '#f8fafc' }}>
                      <th style={{ textAlign: 'left', padding: '16px 24px', fontSize: '12px', fontWeight: 700, color: '#64748b', textTransform: 'uppercase' }}>Produto</th>
                      <th style={{ textAlign: 'center', padding: '16px', fontSize: '12px', fontWeight: 700, color: '#64748b', textTransform: 'uppercase' }}>Inicial</th>
                      <th style={{ textAlign: 'center', padding: '16px', fontSize: '12px', fontWeight: 700, color: '#64748b', textTransform: 'uppercase' }}>Entradas</th>
                      <th style={{ textAlign: 'center', padding: '16px', fontSize: '12px', fontWeight: 700, color: '#64748b', textTransform: 'uppercase' }}>Final</th>
                      <th style={{ textAlign: 'center', padding: '16px', fontSize: '12px', fontWeight: 700, color: '#64748b', textTransform: 'uppercase' }}>Consumo</th>
                      <th style={{ textAlign: 'center', padding: '16px', fontSize: '12px', fontWeight: 700, color: '#64748b', textTransform: 'uppercase' }}>Custo (R$)</th>
                    </tr>
                  </thead>
                  <tbody>
                    {filteredProducts.map((p) => {
                      ...
                      <tr key={p.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                        ...
                        <td style={{ textAlign: 'center', padding: '16px' }}>
                          <div style={{ color: consumo > 0 ? '#f97316' : '#94a3b8', fontWeight: 700 }}>{consumo.toFixed(2)}</div>
                        </td>
                        <td style={{ textAlign: 'center', padding: '16px' }}>
                          <div style={{ color: custo > 0 ? '#ef4444' : '#94a3b8', fontWeight: 700 }}>R$ {custo.toFixed(2)}</div>
                        </td>
                      </tr>
                    )})}
                  </tbody>
                    ...
                    <div key={p.id} className="bg-slate-50 p-4 rounded-xl border border-slate-200 shadow-sm">
                      ...
                      <div className="flex justify-between border-t border-slate-200 pt-3">
                        <div className="flex items-center gap-2">
                          <span className="text-[11px] uppercase font-bold text-slate-400">Consumo:</span>
